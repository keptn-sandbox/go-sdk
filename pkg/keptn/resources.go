...
func (r *ResourceHandler) writeResources(uri string, method string, resources []*Resource) (string, error) {
	...
	copiedResources := make([]*Resource, len(resources), len(resources))
	for i, val := range resources {
		copiedResources[i] = &Resource{ResourceURI: val.ResourceURI, ResourceContent: b64.StdEncoding.EncodeToString([]byte(val.ResourceContent))}
	}
...
func (r *ResourceHandler) writeResource(uri string, method string, resource *Resource) (string, error) {
	...
	copiedResource := &Resource{ResourceURI: resource.ResourceURI, ResourceContent: b64.StdEncoding.EncodeToString([]byte(resource.ResourceContent))}
...
func (r *ResourceHandler) getAllResources(u *url.URL) ([]*Resource, error) {
